...
import { Button, classes } from "@/mdxui/button"
import { CopyToClipboard } from "@/mdxui/copy-to-clipboard"

export function Pre({ children, className, "data-filename": filename, "data-copy": copy, "data-language": language, icon: Icon, style, ...props }) {
    const preRef = useRef(true)

    const toggleWordWrap = useCallback(() => {
        const htmlDataset = document.documentElement.dataset
        const hasWordWrap = "openhumanWordWrap" in htmlDataset
        if (hasWordWrap) {
            delete htmlDataset.openhumanWordWrap
        } else {
            htmlDataset.openhumanWordWrap = ""
        }
    }, [])
    ...
    return (
        <div className="openhuman-code relative mt-6 first:mt-0">
            ...
            <div className={cn("opacity-0 transition [div:hover>&]:opacity-100 focus-within:opacity-100", "flex gap-1 absolute right-4", filename ? "top-14" : "top-2")}>
                <Button onClick={toggleWordWrap} className="md:hidden bg-red-500" title="Toggle word wrap">
                    <svg viewBox="0 0 24 24" width="24" height="24" fill="currentColor" className="h-4 w-auto">
