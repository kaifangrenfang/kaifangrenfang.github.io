...
const arrowEl = <span className="transition-transform duration-75 group-hover:translate-x-[2px]">→</span>
...
export function Card({ children, title, icon, image, arrow, href, ...props }) {
    ...
    return (
        <Link
            ...
            {...props}
        >
            <span className={cn(classes.title, "dark:text-neutral-200 dark:hover:text-neutral-50 flex items-center")}>
                {icon}
                {title}
                {animatedArrow}
            </span>
...
export function Cards({ children, num = 3, className, style, ...props }) {
    return (
        <div
