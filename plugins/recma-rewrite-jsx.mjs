...
export const recmaRewriteJsx = () => (ast, file) => {
    ast.body = ast.body
        ...
        .filter((node) => node.type !== "ExportDefaultDeclaration" || node.declaration.name !== "MDXContent")

    const createMdxContentIndex = ast.body.findIndex((o) => o.type === "FunctionDeclaration" && o.id.name === "_createMdxContent")
    ...
    const returnStatement = createMdxContent.body.body.find((o) => o.type === "ReturnStatement")
    ...
    visit({ children: returnBody }, "JSXElement", (heading) => {
        ...
        const idNode = openingElement.attributes.find((attr) => attr.name.name === "id")
        if (!idNode) return
