                ...
                body: [
                    {
                        ...
                        source: { type: "Literal", value: "@/icons" },
                        specifiers: [
                            {
...
const isImportFrom = (node) => node.data.estree.body[0].source.value === "@/icons"

export const rehypeIcon =
    (replaces = REHYPE_ICON_DEFAULT_REPLACES) =>
    (ast) => {
        const imports = ast.children.filter((node) => isMdxJsEsm(node) && isImportDeclaration(node) && isImportFrom(node))
