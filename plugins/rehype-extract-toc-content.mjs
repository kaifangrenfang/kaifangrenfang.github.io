...
export const rehypeExtractTocContent = () => (ast, file) => {
    ...
    const TocToExpression = Object.fromEntries(toc.map((node) => [node.properties.id, toEstree(node).body[0].expression]))
    ...
    const elements = (file.data.toc || []).map((n) => {
        ...
        const isText = node.children.every((child) => child.type === "JSXExpressionContainer" && child.expression.type === "Literal")
        ...
        return createAstObject({
            value: result,
            id: node.openingElement.attributes.find((attr) => attr.name.name === "id").value.value,
