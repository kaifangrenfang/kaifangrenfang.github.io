...
[![](../images/filament_logo.png)](../images/filament_logo.png)
...
In \[[Lagarde18](#citation-lagarde18)\], with credit to Emmanuel Turquin, Lagarde and Golubev make the observation that equation $\ref{averageFresnel}$ can be simplified to $f_{0}$. They also propose to apply energy compensation by adding a scaled GGX specular lobe:
...
We can implement specular energy compensation at a negligible cost if we store $r$ in the DFG lookup table presented in section [5.3](#toc5.3). [Listing 10](#listing_energycompensationimpl) shows that the implementation is a direct conversion of equation $\ref{scaledEnergyCompensationLobe}$.
...
Defines how much of the ambient light is accessible to a surface point. It is a per-pixel shadowing factor between 0.0 and 1.0. This parameter will be discussed in more details in the [lighting](#lighting) section
...
\[[Estevez17](#citation-estevez17)\] also presents a new shadowing term that we omit here because of its cost. We instead rely on the visibility term from \[[Neubelt13](#citation-neubelt13)\] (shown in equation $\ref{clothSpecularBRDF}$ above). The implementation of this NDF is presented in [listing 18](#listing_clothcharliebrdf), optimized to properly fit in half float formats.
...
Equation $\ref{spotLightLuminousPower}$ describes how the luminous power of a spot light can be calculated in a similar fashion to point lights, using $ \theta\_{outer} $ the outer angle of the spot light's cone in the range \[0..$\pi$\].
...
The easiest way to compute RGB values from this locus is to use the formula described in \[[Krystek85](#citation-krystek85)\]. Krystek's algorithm (equation $\ref{krystek}$) works in the CIE 1960 (UCS) space, using the following formula where $T$ is the desired temperature, and $u$ and $v$ the coordinates in UCS.
...
For reference, [figure 47](#figure_colortemperaturescalecctclamped) shows the range of correlated color temperatures without the normalization step presented in equation $\ref{normalizedRGB}$.
...
$$
\tilde{I}(f(\Theta)L_{\bot}) &= \tilde{I}\left\[f\left(\Theta\right) \left(\bar{L_{\bot}} + \DeltaL_{\bot}\right)\right\] \\ &= K \times \hat{I}\left\[f\left(\Theta\right) \left(\bar{L_{\bot}} + \DeltaL_{\bot}\right)\right\] \\ &= K \times \left\[\hat{I}\left(f\left(\Theta\right)\bar{L_{\bot}}\right) + \hat{I}\left(f\left(\Theta\right)\DeltaL_{\bot}\right)\right\] \\ &= K \times \hat{I}\left(f\left(\Theta\right)\bar{L_{\bot}}\right) + K \times \hat{I}\left(f\left(\Theta\right) \DeltaL_{\bot}\right) \\ &= \tilde{I}\left(f\left(\Theta\right)\bar{L_{\bot}}\right) + \tilde{I}\left(f\left(\Theta\right) \DeltaL_{\bot}\right) \\ &= I\left(f\left(\Theta\right)\bar{L_{\bot}}\right) + \tilde{I}\left(f\left(\Theta\right) \DeltaL_{\bot}\right)
$$
...
Since we know that EV is a base-2 logarithmic scale in which each stop increases or decreases the brightness by a factor of 2, we can formally define $ EV\_{S} $, the exposure value at given sensitivity (equation $\ref{evS}$).
...
Calculating the $ EV\_{100} $ as a function of the 3 camera settings is trivial, as shown in $\ref{ev100}$.
...
Since we want to work with $ EV\_{100} $, we can substitute $K$ and $S$ in equation $\ref{evK}$ to obtain equation $\ref{ev100L}$.
...
Since we want to work with $ EV\_{100} $, we can substitute $S$ $\ref{evC}$ to obtain equation $\ref{ev100C}$.
...
We choose the saturation-based speed relation, which gives us $ H\_{sat} $, the maximum possible exposure that does not lead to clipped or bloomed camera output (equation $\ref{hSat}$).
...
We combine equations $\ref{hSat}$ and $\ref{photometricExposure}$ in equation $\ref{lmax}$ to compute the maximum luminance $ L\_{max} $ that will saturate the sensor given exposure settings $S$, $N$ and $t$.
...
$ L\_{max} $ can be simplified using equation $\ref{ev}$, $ S = 100 $ and $ q = 0.65 $:
...
With $c$ the bloom color and $ EV\_{100} $ the current exposure value, we can easily compute the luminance of the bloom value as show in equation $\ref{bloomEV}$.
...
Equation $\ref{bloomEV}$ can be used in a fragment shader to implement emissive blooms, as shown in [listing 43](#listing_fragmentemissive).
...
The evaluation of $ L\_{out}(n,v,\Theta) $ with importance sampling is presented in equation $\ref{annexIblImportanceSampling}$.
...
In equation $\ref{annexIblImportanceSampling}$, $p$ is the probability density function (PDF) of the distribution of _important direction samples_ $l_i$. These samples depend on $h_i$, $v$ and $\alpha$. The definition of the PDF is shown in equation $\ref{iblPDF}$.
...
Refer to section [9.3](#toc9.3) for more details. Given a uniform distribution $(\zeta_{\phi},\zeta_{\theta})$ the important direction $l$ is defined by equation $\ref{importantDirection}$.
...
The term $ L*{DFG} $ is only dependent on $ n \cdot v $. Below, the normal is arbitrarily set to $ n=\left\[0, 0, 1\right\] $ and $v$ is chosen to satisfy $ n \cdot v $. The vector $ h_i $ is the $ D*{GGX}(\alpha) $ important direction sample $i$.
