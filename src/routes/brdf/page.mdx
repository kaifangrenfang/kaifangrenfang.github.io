...
Visit Origin Posts: [google.github.io/Filament](https://google.github.io/filament/Filament.md.html)
...
Finally, we can show that the scale factor $K$ satisfies our average irradiance ($\bar{L_{\bot}}$) requirement by plugging $L_{\bot} = \bar{L_{\bot}} + (L_{\bot} - \bar{L_{\bot}}) = \bar{L_{\bot}} + \Delta L_{\bot}$ into $\tilde{I}$:
...
A way to think about this approximation is that it splits the radiance $L_{\bot}$ in two parts, the average $\bar{L_{\bot}}$ and the delta from the average $\Delta L_{\bot}$ and computes the correct integration of the average part then adds the simplified integration of the delta part:
