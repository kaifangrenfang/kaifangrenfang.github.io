...
export default {
    project: {
        link: "https://github.com/shuding/openhuman",
    },
    logo: <b>Project</b>,
}
```
...
```js
export default {
    docsRepositoryBase: "https://github.com/shuding/openhuman/tree/main/docs",
}
```

Then openhuman will automatically generate the correct file path for all pages.
...
export default {
    head: (
        <>
            ...
            <meta property="og:title" content="openhuman" />
...
```jsx
import { useRouter } from "next/router"
import { useConfig } from "openhuman-theme-docs"
    ...
    head: () => {
        ...
        return (
            <>
                ...
                <meta property="og:title" content={frontMatter.title || "openhuman"} />
...
<div className="mt-8 text-center text-sm">[Live example on StackBlitz ↗](https://stackblitz.com/edit/openhuman-2-docs-yrlccm?file=theme.config.jsx)</div>
...
it links to openhuman’s GitHub repository.
...
A banner can be dismissed. By default `banner.key` will be `"openhuman-banner"` and
it’s used by
...
export default {
    banner: {
        key: "2.0-release",
        text: (
            <a href="https://openhuman.site" target="_blank">
                🎉 openhuman 2.0 is released. Read more →
            </a>
        ),
    },
}
...
import { Tabs, Tab } from "@/mdxui"

<Screenshot src={titleSuffixImage} alt="Customized Sidebar" />

<div className="mt-8 text-center text-sm">[Live example on StackBlitz ↗](https://stackblitz.com/edit/openhuman-2-docs-8xbfk3?file=pages%2F_meta.json,theme.config.jsx)</div>
...
[example](https://github.com/shuding/openhuman/issues/new?title=Feedback%20for%20%E2%80%9CTheme%20Configuration%E2%80%9D&labels=feedback).
...
<OptionTable options={[["faviconGlyph", `string`, "The glyph to use as the favicon."]]} />@/mdxui/screenshot@/mdxui@/mdxui@/mdxui
